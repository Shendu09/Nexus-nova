...
AWS Lambda (container image)
    ...
    └─► Voice pipeline (optional):
        ...
        ├─► Polly reads the RCA briefing aloud
        └─► Nova 2 Sonic handles interactive voice investigation
```
...
The template defaults to a pre-built container image published to public ECR. No Docker or image building required. No triggers are enabled by default -- you choose which ones to activate.
...
| `EcrImageUri` | public ECR image | Container image URI (override to use your own build) |
...
When `ConnectEnabled=true`, Flare adds a voice layer on top of the analysis pipeline. After generating the RCA:
...
3. **Briefing**: Polly reads the RCA summary aloud when the engineer answers
4. **Investigation**: Nova 2 Sonic (via Lex V2) handles interactive voice conversation -- the engineer asks questions and gets intelligent, contextual answers powered by the retrieve-then-reason pattern

The voice pipeline requires an Amazon Connect instance, a Lex V2 bot with Nova 2 Sonic speech-to-speech, and a contact flow. These are configured through the AWS console. See the [Voice Setup Guide](docs/voice-setup-guide.md) for step-by-step instructions.
...
**Voice handler** (`src/flare/voice_handler.py`) provides two Lambda entry points: a briefing handler (reads the RCA for the Connect contact flow) and a fulfillment handler (answers follow-up questions using the retrieve-then-reason pattern with cached data and Nova 2 Lite).
